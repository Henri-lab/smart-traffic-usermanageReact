<template>
  ...
  import { useEventStore } from "@/stores/addEventDialog.js";
  import { ElMessage } from "element-plus";
  import { addPoint } from "../../ol-utils/add-event.js";
  ...
  const addEvent = () => {
    ...
    // 移除地图点击事件
    console.log($map,'789789789');
    $map.getInteractions().pop();
    // 1 坐标位置
    let position = eventStore.position;
    // 2 目标图层
    const layer = new Zondy.Map.Doc("", "guanggu", {});
    $map.addLayer(layer);
    // 3 属性集
    // 4 服务对象
    const service = {
